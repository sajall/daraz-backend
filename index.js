...
// finding the user based on the id that was sent to signup page on the click of edit button
// after finding the user based on id i sent the user back to the signup page and signup page used that user
// for reseting values in the signup form again
// app.get("/user-to-update", async (req, res) => {
//   //  console.log(req.query.id);
//   const user = await usersModel.findOne({ _id: req.query.id });
//   res.json(user);
// });
...
app.put("/find-user", async (req, res) => {
  // console.log(req.body , 'this is request body here');
  try {
  ...
  //   });
  // res.json({sucess: true , message:"user found"  });
  // jsonwebtoken
  //   console.log(user, "this is my user for checking email");
});
...
app.post("/check-token", (req, res) => {
  ...
  // let decoded = jsonwebtoken.verify(req.body.token, " my name is amina");
